...
std::vector<RecognizeFn> buildRegistry()
{
    ...
    // Freeform DOME boss — a spherical-cap dome (sapphire glass / sensor dome)
    // built by pr::domeSolid (a BSpline loft).  Recognised by its freeform cap
    // face(s) + spherical-cap volume gate; regenerated by the dome_boss skill.
    reg.emplace_back(&skill::dome_boss::recognize);

    // ANNULAR groove — a plain ring channel (watch bezel pocket, phone camera
    // deco ring).  Recognised by a coaxial cylinder pair joined by a flat
...
const std::unordered_set<std::string>& preciseRecognizers()
{
    static const std::unordered_set<std::string> kPrecise = {
        ...
        "linear_pattern",
        "circular_pattern",
        // Dome boss: the geometric path requires freeform cap face(s) whose
        // surface area matches a spherical cap of the fitted (base radius, rise)
        // — a non-dome loft/blob fails the area gate — so a recovered dome is
        // specific enough to keep full confidence and reach the Executor.
        "dome_boss",
        // Annular groove: the geometric path requires a coaxial cylinder pair
        // joined by a flat ANNULAR floor (concentric inner+outer circular edges
        // on one axis-normal plane).  That floor gate rejects an O-ring U-groove
