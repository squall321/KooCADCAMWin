// @lat: [[engine/skills#body_combine_union]]

#include "body_combine_union.hpp"
...
namespace koocadcam::skill::body_combine_union {
...
TopoDS_Shape buildAux(const Input& in)
{
    ...
    throw SkillError("body_combine_union: unknown aux_kind '" + in.aux_kind + "'");
}
...
SkillOutput apply(const Workpiece& wp, const Input& in)
{
    ...
    if (!dfm.passed) {
        std::string msg = "body_combine_union DFM failed:";
    ...
    const TopoDS_Shape newShape = pr::fuse(wp.shape(), aux);
    ...
    json pattern = {
        ...
        { "derived_added_volume_mm3",    added },
        { "volume_before_mm3",           volBefore },
        { "volume_after_mm3",            volAfter },
    };
    ToolingMeta tooling;
    tooling.tool_type         = "additive_fixture";
    tooling.stock_removed_mm3 = -added;     // negative = stock ADDED
    tooling.est_cycle_time_s  = std::max(1.0, std::abs(added) / 5000.0);
    ...
    spdlog::debug("skill::body_combine_union: vol {} → {} (+{})",
                  volBefore, volAfter, added);
