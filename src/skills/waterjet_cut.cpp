...
namespace koocadcam::skill::waterjet_cut {
...
sc::ProcessLimits processLimits()
{
    return sc::ProcessLimits{
        ...
        /*tool_type*/        "waterjet",
        /*process_kerf_mm*/  0.9,    // bin centroid
    };
}
...
SkillOutput apply(const Workpiece& wp, const Input& in)
{
    ...
    if (!dfm.passed) {
        std::string msg = "waterjet_cut DFM failed:";
    ...
    if (!entryId) throw SkillError("waterjet_cut: entry_face datum unresolved");
    ...
    // Waterjet is SLOW: ~5 mm/s in 25 mm steel, slower in thicker stock.
    const double pathLen   = sc::pathLength(in);
    const double feedSpeed = std::max(1.0, 5.0 * (25.0 / std::max(1.0, thickness)));
    sig.tooling.est_cycle_time_s  = std::max(1.0, pathLen / feedSpeed);
    sig.tooling.tool_material     = "garnet_abrasive";
    sig.tooling.cutting_speed_sfm = 0.0;   // not a rotational tool
    sig.tooling.feed_per_tooth_mm = 0.0;
    sig.tooling.flute_count       = 0;
    sig.tooling.extra["machining_constraint"] =
        "Abrasive waterjet — smooth edge, no HAZ, slow in thick stock; "
        "through-cut only.";
