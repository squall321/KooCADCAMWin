...
#include "sweep_boss_along_curve.hpp"
...
namespace koocadcam::skill::sweep_boss_along_curve {
...
// Shoelace area of a closed XY polyline (unsigned).
...
// Build a closed planar face from a 2D polygon, positioned at `origin` with
// the polygon's local +Z aligned to `normal`.
TopoDS_Face makeProfileFace(const std::vector<std::pair<double,double>>& poly,
                            const gp_Pnt& origin,
                            const gp_Dir& normal)
{
    // Trim trailing duplicate-of-first.
    ...
    for (std::size_t i = 0; i < end; ++i) {
        ...
        // Build edge on the plane using planar 2D coords.
        // We project to 3D via the plane's local XY axes.
...
SkillOutput apply(const Workpiece& wp, const Input& in)
{
    ...
    if (!dfm.passed) {
        std::string msg = "sweep_boss_along_curve DFM failed:";
    ...
    try {
        ...
        throw SkillError(std::string("sweep_boss_along_curve: ") + ex.what());
    }

    // 2) profile face at pts[0] normal to first segment
    gp_Vec firstSeg(pts[0], pts[1]);
    if (firstSeg.Magnitude() < 1e-9) {
        throw SkillError("sweep_boss_along_curve: zero-length first segment");
    }
    ...
    try {
        ...
        spdlog::debug("sweep_boss_along_curve: pipe threw — {}", ex.what());
    }
    ...
    ToolingMeta tooling;
    tooling.tool_type         = "swept_boss";
    tooling.stock_removed_mm3 = -expectedV;   // material ADDED
    tooling.est_cycle_time_s  = std::max(1.0, L / 50.0);
    ...
    spdlog::debug("skill::sweep_boss_along_curve: A={} L={} added={} done={}",
                  A, L, addedVol, sweepOk);
...
std::vector<RecognizedFeature> recognize(const Workpiece& wp)
{
    ...
    // (2) geometric: count ruled (extruded/linear-blend) faces. A sweep
    // along a polyline leaves several non-planar non-cylindrical ruled
    // faces; if we see ≥ 1 such face, emit a low-confidence guess.
    int ruledLike = 0;
    ...
    if (ruledLike >= 1) {
        ...
        rf.confidence       = 0.45;     // geometric-only guess
