...
namespace koocadcam::skill::mig_weld {
...
DFMReport validate(const Workpiece& wp, const Input& in)
{
    ...
    if (in.bead_width_mm <= 0.0) {
        r.add("DFM-INPUT", "error", "mig_weld bead_width_mm must be > 0");
    }
    if (in.bead_height_mm <= 0.0) {
        r.add("DFM-INPUT", "error", "mig_weld bead_height_mm must be > 0");
    }
    if (in.bead_width_mm > 0.0 && in.bead_width_mm < 2.0) {
        r.add("DFM-MIG-WIDTH", "error",
              "mig_weld bead_width_mm " + std::to_string(in.bead_width_mm) +
              " < 2.0 mm (use TIG for narrow beads)");
    }
    if (in.bead_width_mm > 12.0) {
        r.add("DFM-MIG-WIDTH", "error",
              "mig_weld bead_width_mm " + std::to_string(in.bead_width_mm) +
              " > 12.0 mm (multi-pass required)");
    }
    if (in.wire_dia_mm < 0.6) {
        r.add("DFM-MIG-WIRE", "error",
              "mig_weld wire_dia_mm " + std::to_string(in.wire_dia_mm) +
              " < 0.6 mm (sub-spec consumable)");
    }
    if (in.wire_dia_mm > 1.6) {
        r.add("DFM-MIG-WIRE", "error",
              "mig_weld wire_dia_mm " + std::to_string(in.wire_dia_mm) +
              " > 1.6 mm (heavy-section process, use submerged arc)");
    }
    if (in.bead_width_mm > 0.0 &&
        in.bead_height_mm > in.bead_width_mm * 0.5) {
        r.add("DFM-MIG-RATIO", "error",
              "mig_weld bead_height " + std::to_string(in.bead_height_mm) +
              " > 0.5 × bead_width " + std::to_string(in.bead_width_mm) +
              " — multi-pass build-up, not a single MIG pass");
    }
    const double seamLen = std::hypot(in.end_x_mm - in.start_x_mm,
                                      in.end_y_mm - in.start_y_mm);
    if (seamLen <= 1e-6) {
        r.add("DFM-INPUT", "error", "mig_weld start and end must differ");
    }
    if (in.bead_width_mm > 0.0 && seamLen > 0.0 && seamLen <= in.bead_width_mm) {
        r.add("DFM-MIG-LEN", "warning",
              "mig_weld length " + std::to_string(seamLen) +
              " ≤ bead_width " + std::to_string(in.bead_width_mm) +
              " — consider a tack weld");
    }
    if (in.wire_feed_mm_per_s <= 0.0) {
        r.add("DFM-MIG-FEED", "warning",
              "mig_weld wire_feed_mm_per_s must be > 0");
    }
    (void)wp;
    return r;
}
...
SkillOutput apply(const Workpiece& wp, const Input& in)
{
    ...
    if (!dfm.passed) {
        std::string msg = "mig_weld DFM failed:";
    ...
    if (!entryId) throw SkillError("mig_weld: entry_face datum unresolved");
    if (!wp.isFacePlanar(*entryId))
        throw SkillError("mig_weld: entry_face must be planar");
    ...
    json params = {
        ...
        { "wire_feed_mm_per_s",  in.wire_feed_mm_per_s },
    ...
    ToolingMeta tooling;
    tooling.tool_type         = "mig_torch";
    tooling.tool_dia_mm       = in.bead_width_mm;
    tooling.tool_length_mm    = 0.0;
    tooling.tool_material     = "steel_wire";   // consumable wire
    ...
    // MIG travel speed ~ 25 mm/s — faster than TIG but slower than laser.
    tooling.est_cycle_time_s  = std::max(1.0, seamLen / 25.0);
    tooling.extra = {
        { "process",            "mig" },
        { "material",           in.material },
        { "gas",                in.gas },
    ...
    spdlog::debug("skill::mig_weld applied: len={} w={} h={} wire_dia={} faces {}→{}",
                  seamLen, in.bead_width_mm, in.bead_height_mm, in.wire_dia_mm,
