...
**3번째 패턴 — rectangular_hole_grid (2026-06-14f):** 같은 atom·revolution 게이트, matchHolePatterns 에 grid 분기 추가(circle→line→grid 순, 각 분기 자체완결 — 실패시 fall-through). `fitGridXY`: corner-seed(min x,y) + nearest-neighbor 로 격자 basis u·v 추정(비정방 피치도 robust — u=최근접, v=u와 수직성분 실재하는 최근접), Cramer 로 각 점 (i,j) 정수복원, 잔차<0.3mm + **FULL 사각형(cols·rows==n, cols·rows≥2, n≥6) + 셀 중복 없음** 게이트. **2×2(=4 concyclic)는 의도적으로 grid 아님 → bolt_circle 이 선점**(grid 는 ≥2×3). recovered={cols,rows,pitch_u/v}. 테스트 13→**18**(2×3/3×3 정방/2×2-는-볼트서클/불완전격자-거부/STEP 왕복 e2e 2×3). **워치 스피커 그릴(step7)이 바로 이 패턴** → RE 가 자기 제품을 인식. fpscan spurious 0. 남은 일반화: 동축 step-bore[M, counterbore/bore atom 에 wrap/축 publish 선행], flange bolt ring[M, drill_hole entry-plane atom], + hollow_cavity 생존자(B3.4 clamp 연결) + B1.3 DSL(L, 패턴 더 손코딩 후 데이터화).
