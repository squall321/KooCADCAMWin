...
**정공법 구현 — 선언형 compound grammar 1번 패턴 (2026-06-14d):** B1.2 SPECIFIC 경로를 코드로 착수. `src/re/CompoundGrammar.{hpp,cpp}` — analyze() 가 복구한 **precise atom** 위에서, 선언된 패턴이 정확히 맞을 때만 grounded COMPOUND 후보를 발화하는 순수 함수(Workpiece 비접근). analyze() 의 cap 루프 **이후** 배선되어 grounded compound 는 데모트 면제(loose 기하 폴백이 아니라 측정 atom 기반이므로 0.5 캡 미적용). 첫 패턴 = **bolt-circle**(동반경·평행축 drill 홀 ≥3 이 공통 원 위, residual<0.3mm). **fpscan 가 즉시 FP 적발**: rect_pocket 의 코너 필렛 4개가 사각형 외접원에 concyclic → "4 볼트홀"과 중심점 기하가 동일 → 첫 구현이 rect_pocket 에 발화. **해결(원리적·일반화)**: drill_hole 가 각 atom 에 `cyl_wrap_rad`(실린더 면 wrap) 태깅 → grammar 가 atom 을 위치/축/직경으로 **physical hole 로 병합**하고 distinct-face wrap 합산, **완전 revolution(≥1.5π)인 홀만** 채택(코너 필렛=90°=0.5π 거부; STEP 분할로 2×180°/4×90° 된 진짜 홀은 합산 ~2π 통과). drill_through_hole 는 drill_hole 의 matched_geometry 를 상속하므로 태깅 1곳이 양쪽 적용. **검증 = `tests/re/compound_grammar_test`(9 케이스)**: recall(4·6홀 링, STEP 왕복 e2e: hole_count·bolt_circle_dia·중심 복구, cap 면제로 conf≥0.7 유지), specificity(2홀/혼합직경/공선/**concyclic 코너 필렛**/저신뢰 atom 전부 무발화), 단일홀 안전. fpscan CapDemotes 그린 복귀. **의의: foreign CAD 에서 domain-compound 인식이 처음으로 안전하게 동작(밴드에이드 아닌 패턴 레지스트리 — 다음 패턴은 함수 1개 추가).**

**2번째 패턴 — linear_hole_array (2026-06-14e, audit 최고 레버리지 권고 실행):** 같은 drill atom + 같은 revolution-completeness 게이트 재사용(새 atom 0), `matchBoltCircles`→`matchHolePatterns` 로 통합해 그룹당 circle 먼저 시도 후 line 폴백(둘은 기하적 상호배타 — concyclic≠collinear, circle fit 은 직선에서 degenerate). 안전 판별자: 동일경+축평행+**PCA 라인핏 수직잔차<0.3mm**(공선)+**균일 피치**(편차<max(0.1mm,6%))+n≥3 — bolt_circle 이 명시적으로 reject 하는 collinear 케이스를 보완. recovered={hole_count,pitch_mm,span_mm,start_x/y,direction}. 테스트 9→**13**(직선 5홀/대각 4홀/불균일 거부/볼트서클-비혼동 추가), fpscan 패널 spurious linear array 0(생존자 표 5 불변). **패턴 레지스트리가 "일반화"임을 2 패턴으로 실증.**

**3번째 패턴 — rectangular_hole_grid (2026-06-14f):** 같은 atom·revolution 게이트, matchHolePatterns 에 grid 분기 추가(circle→line→grid 순, 각 분기 자체완결 — 실패시 fall-through). `fitGridXY`: corner-seed(min x,y) + nearest-neighbor 로 격자 basis u·v 추정(비정방 피치도 robust — u=최근접, v=u와 수직성분 실재하는 최근접), Cramer 로 각 점 (i,j) 정수복원, 잔차<0.3mm + **FULL 사각형(cols·rows==n, cols·rows≥2, n≥6) + 셀 중복 없음** 게이트. **2×2(=4 concyclic)는 의도적으로 grid 아님 → bolt_circle 이 선점**(grid 는 ≥2×3). recovered={cols,rows,pitch_u/v}. 테스트 13→**18**(2×3/3×3 정방/2×2-는-볼트서클/불완전격자-거부/STEP 왕복 e2e 2×3). **워치 스피커 그릴(step7)이 바로 이 패턴** → RE 가 자기 제품을 인식. fpscan spurious 0.

**4번째 패턴 — coaxial_step_bore (2026-06-14g):** audit 가 [M, 선행조건]으로 봤으나 `bore_cylindrical::recognize` 가 이미 axis/position/diameter/depth publish → 실제 [S]. bore_cylindrical 에 `cyl_wrap_rad` 태깅 추가(drill_hole 과 동일, partial 오목 arc 거부용). 매처: assembleHoles 에 `includeBores` 플래그 → drill+bore atom 수집, **같은 축선(coaxialLines: 평행축+축간 수직거리<0.3mm) 위 revolution-complete 실린더 ≥3개 서로 다른 직경** → step bore. equal-dia 홀패턴(circle/line/grid)과 직교(그쪽은 같은 dia 다른 위치, 이쪽은 다른 dia 같은 축). 2-step(plain counterbore)은 bore_with_shelf 영역이라 <3 무발화 → multi_step_bore(현재 캡) 개념을 안전 un-cap. recovered={step_count,diameters_mm[],max/min_dia,position,axis}. 테스트 18→**22**(3-step/2-step-거부/평행등경-거부/drill+bore STEP 왕복 e2e). fpscan spurious 0(패널 최대 2 동축단차). **그래머 4 패턴(circle/line/grid/step-bore) 안정.**

**#3 스킵(중복) + #4 hollow_cavity grounding (2026-06-14h):** #3 flange_bolt_ring/counterbored_bolt_circle 은 bolt_circle 이 이미 같은 원을 인식(counterbore pilot→drill_hole→bolt_circle) + counterbore atom 도 개별 인식 → dedup 복잡성만 추가하는 저가치라 **스킵**(밴드에이드 회피). 대신 실제 프런티어 = fpscan 생존자 grounding. **hollow_cavity thin-wall 게이트**: 진짜 cavity 는 쉘(wall≪opening), 솔리드 블록 pocket/bore 는 wall≥opening → `gMean > 0.35·cavMin` 이면 거부(pocket/bore 는 mill_*_pocket/bore_* 가 기술). 메타데이터 replay 경로는 이 게이트 위에서 early-return 이라 자가 빌드 박벽 쉘은 영향 없음. **측정 결과 fpscan 생존자 5→3**: hollow_cavity 제거 + bezel_groove_assembly 동반 제거(그 recognizer 가 hollow_cavity 내부 의존 → 한 게이트로 2개 grounding). bearing seat(wall26.5/cav47=0.56)도 거부 → bearing_housing_test 의 수동 hollow_cavity drop 근본원인 해소. hollow_cavity 스킬 테스트 5/5 유지(fixture wall/cav≤0.09). 남은 생존자 3: auto_rib_between_two_walls(2/13)·gas_strut_hinge_compound(1/13)·tube_swage(9raw/1생존).

**잔여 생존자 grounding + un-cap no-op 결론 (2026-06-15b — 3트랙 "셋다"):** **tube_swage 완전 grounding**: recognizer 가 concavity 체크 부재(drill/bore 의 isBoreWall 누락) → 내부 보어(오목) 전부에 발화. `isExternalCylinder`(외향 법선=convex) 게이트 추가 → 실 swage(볼록)는 통과, 내부 보어 거부. **auto_rib 슬랩-FP grounding**: 두 반평행 평면 gap 이 파트 normal-extent 의 0.5배 초과면 거부(thin_plate 4mm=전두께는 rib 아닌 슬랩). 두 스킬 자기테스트 5/5 유지. **생존자 5→2**(tube_swage 제거, auto_rib 슬랩케이스 제거; 잔여 auto_rib 1·gas_strut 1). **gas_strut 은 deferred**: "유사반경·횡분리 Z-보어 2개"가 실제 시그니처라 "무관한 구멍 2개"(multi 파트의 counterbore pilot+drill)와 기하적으로 동일 — 반경게이트 0.3 축소는 이 패널만 patch 하는 밴드에이드라 거부, richer context 필요. **Track B(dedupe-subsumed 9개 un-cap) = 측정상 NO-OP 으로 결론**: fpscan survives-dedupe=0 이 곧 "un-cap 해도 dedupe 가 즉시 재드롭→출력 불변"의 증거(구조적: 그 compound face set ⊆ precise atom face set). 즉 un-cap-safe 한 것은 redundant, 가치있는 것(생존자)은 unsafe — 이것이 캡이 load-bearing 인 근본 이유. **안전한 un-cap 의 유일한 길 = 구체적 grammar(4패턴) + recognizer grounding(Track A), 캡 플립 아님.**

**Track C — B3.4 ParamClamp 레이어 (2026-06-15c):** RE 복원 ProcessPlan 을 fresh stock 에 재실행할 때 over-spec 파라미터가 apply() 를 crash 시키는 문제(예: hollow_cavity wall_thickness>stock)를 일반 해결. 신규 `src/process/ParamClamp.{hpp,cpp}`: `clampTable()` 레지스트리(skill_id→ClampFn(params,stock)) + `clampParams()`. Executor::execute 디스패치 직전(Executor.cpp:4956) 1줄 훅 — 엔트리 없는 스킬은 no-op, clamp 는 never-throw 라 700+ 스킬에 무해. 첫 엔트리 hollow_cavity: wall<0.45·min(XY), depth<Zspan−0.4(0.4mm DFM 플로어 존중). 검증 `tests/process/param_clamp_test`(5): clamp 값 정확 + no-rule passthrough + valid 미변경 + **raw apply 는 throw / Executor 경유는 clamp 후 성공**(실제 dispatch 경로 증명). bearing_housing_test 의 수동 hollow_cavity drop 은 thin-wall 게이트로 이미 무력화돼 그대로 둠(이제 일반 레이어가 대체). **3트랙 완료**: A(생존자 5→2 grounding) + B(un-cap no-op 측정 결론) + C(replay clamp 인프라). 남은 RE 프런티어 = gas_strut/auto_rib-잔여(inherently ambiguous, richer context 대기) / B1.3 DSL(미도래).
...
**B6.4 — 폰 GUI 에디터 (2026-06-16, 최고 ROI 선택):** 2-에이전트 ROI 조사로 B9.3(anchorPart 랭킹=consumer 부재로 가치 deferred) 대신 **폰 GUI** 선택(완성된 폰 엔진을 편집+DFM 가능한 실 제품으로, 워치 패턴 미러, headless 테스트 템플릿 존재→검증 강함). **신규 `src/gui/PhonePanel.{hpp,cpp}`**: 워치 패널과 달리 **완전 스키마-구동**(FieldSchema 팩토리로 group 4종[base/corner_radius/display_pocket/port_hole] + array table 2종[cameras/camera_deco_rings] 빌드, currentSpec/setSpec 제네릭 루프). side_buttons(string-enum)는 m_baseSpec pass-through. **MainWindow**: 폰 dock(워치와 tab) + onSpecChanged 를 **product-aware**(sender() 기반 워치/폰 분기)로, **양 제품 live DFM 점등**(기존 워치 live 루프엔 DFM 없었음 — 보너스). **AppMenus::onNewPhoneRect**: 1회성 view-only → setSpec(defaultSpec)+activatePhone(편집 모드). 검증 `tests/gui/phone_panel_test`(headless offscreen 5): pass-through 보존 / base 위젯 반영 / **round-trip→buildAll→runDFM PASS** / cameras 편집 / deco-ring add-remove. 워치 패널 무변경(blast radius 0). **워치+폰 양 제품이 GUI에서 편집·DFM 게이트 가능.**

**grammar 컴파운드 actionable 화 — 인식→편집→재생성 루프 완성 (2026-06-17, 미션 페이오프):** 추천에 따라 grammar 작업의 빠진 절반 보완. grammar 4패턴은 **인식만** 됐음(apply 부재 → inferProcessPlan Unknown 버킷, replay 시 filterToDispatched 가 드롭). 플래그십 **`bolt_circle_pattern` 을 생성형 스킬로 구현**(`src/skills/bolt_circle_pattern.{hpp,cpp}`): apply 가 drill_hole::apply 를 hole_count 회 합성(entry/through 해석·체인계약 상속), validate(≥3홀·dia·DFM-003 피치갭), recognize 는 {}(인식은 koo_re 그래머). Executor dispatch 배선(parseBoltCirclePattern: axis_dir 로 entry 면 유도) + skills CMake + check_feature_chain 위임 화이트리스트. **recovered_params 키가 그래머 출력과 일치 → 인식된 볼트서클이 replayable·editable 스텝이 됨.** 검증 `tests/skills/bolt_circle_pattern_test`(5, 신규 ctest): 생성(유효 ring·재료제거) / **편집(hole_count 4→6 재생성, 더 많은 재료 제거)** / validate 거부 / **STEP 왕복 재인식(hole_count=4)** / **Executor dispatch 성공**. **의의: 인식된 컴파운드를 파라메트릭하게 편집·재생성 — 일반화된 설계 자동화의 실증.** **이어서 `linear_hole_array` 도 생성형 구현**(bolt_circle 미러, recovered_params 가 start_x/y·direction·pitch·count·dia 완비라 완전 재생성; Executor 배선+위임 화이트리스트; tests/skills/linear_hole_array_test 5: 생성/편집/거부/STEP 재인식/dispatch). **2패턴(bolt_circle·linear) actionable.** **이어서 `rectangular_hole_grid` 도 생성형**: 그래머 보강(fitGridXY 가 계산하던 corner+basis 를 버리지 않고 origin_x/y_mm + u_dir + v_dir 로 recovered_params 에 기록) 후 생성형 스킬(cols×rows 격자 재생성). Executor 배선+위임 화이트리스트, tests/skills/rectangular_hole_grid_test 5(생성/편집 2×3→3×3/거부/STEP 재인식 hole_count=6/dispatch). 그래머 26 회귀 없음. **3패턴(bolt_circle·linear·grid) actionable — 워치 스피커 그릴이 이제 인식→편집 가능.**

**컴파운드 subsumption — 클린 복원 플랜 (2026-06-17):** 복원 플랜이 "N drills + 1 compound"(중복)이던 문제 해결. 그래머가 소비한 홀 중심을 `hole_centers` 로 기록(3패턴 모두), `inferProcessPlan` 이 **dispatchable 홀-패턴 컴파운드(bolt_circle/linear/grid)가 소비한 drill 스텝을 흡수**(B_Hole 버킷에서 컴파운드 중심과 0.5mm 내 drill_hole/drill_through_hole 제거). → 복원 플랜이 컴파운드 **한 개의 편집 가능 단위**가 됨(4볼트→6볼트 편집 시 원래 4 drills 안 남음). dispatchable 패턴만 흡수(step-bore 등 비-dispatchable 은 atom 유지 — 안전). 검증: bolt_circle_pattern_test 에 SubsumesConstituentDrillsInPlan(복원 플랜에 bolt_circle 1 + drill 0). **bearing_housing/watch_re_roundtrip 통합 무영향 확인**(SHCS 피벗이 bolt_circle 로 흡수돼도 closure 유지). **+ capstone end-to-end 테스트**: 인식→복원플랜→replaceParams(4→6)→재실행→6홀 재인식 — 전체 자동화 루프 실증(2804aa7).

**coaxial_step_bore 생성형 — 4패턴 전부 actionable (2026-06-18):** 마지막 그래머 패턴. **그래머 보강**: Hole 에 depth/through 추가(atom depth_mm/through_hole 읽기, 병합 시 max/OR), matchCoaxialStepBores 가 per-step cumulative depth(part-top 기준 = topZ − sec.z + sec.depth, through 는 플래그) 를 `steps:[{diameter_mm,depth_mm,through}]` 로 기록. **생성형 스킬**(`src/skills/coaxial_step_bore.{hpp,cpp}`): 각 스텝 직경을 entry 면에서 cumulative depth 까지 drill_hole 합성(최심 through). Executor 배선+위임 화이트리스트, tests/skills/coaxial_step_bore_test 5(생성/seat 편집/거부/STEP 재인식 step_count=3·max16/dispatch). 그래머 26 회귀 없음. **bolt_circle·linear·grid·step-bore 4패턴 전부 인식↔생성↔편집 가능.**

**step-bore subsumption + 적대적 리뷰 수정 (2026-06-18):** step-bore 도 inferProcessPlan 에서 구성 bore/drill 흡수(중심+축). **13-에이전트 적대적 리뷰 워크플로**(4차원 리뷰→적대적 검증→종합)로 라운드트립 충실도 **실버그 다수 색출**(내 테스트가 through·-Z·각도0 만 써서 못 잡음): **#1 블라인드 홀패턴이 관통으로 둔갑**(3패턴이 depth/through 미기록→Executor 디폴트 through) → depthRep/throughRep 3 emit 추가. **#2 볼트서클 각도 위상 손실**→start_angle_deg(seed 홀 각도) emit. **#4 step-bore 누적깊이 월드Z**→축투영(entryProj)으로 일반화. **#5 과잉-subsumption**(직경 미검사)→hole-pattern drop 에 직경 매칭(±0.2) 가드. **#6 크로스컴파운드 보어 오삭제**(카운터보어 좌면 소실 위험)→bore 삭제는 step-bore 중심에만(dropBores 플래그). **#3 비-Z 축**→3 홀패턴 validate 에 ±Z 전용 가드(인식기가 XY 전용이라 정상워크플로 무영향, 수작업플랜 보호). 회귀 테스트: BlindRing/BlindRow RoundTripsAsBlind, PreservesAngularPhase. **반증된 false-alarm**: Executor `.get<double>()` 크래시(try/catch 로 step-error 격하), 버킷 미스(classify 정확). **교훈: 적대적 멀티에이전트 리뷰가 단위테스트 사각(through/-Z/angle0 가정)의 무음 손상을 잡아냄.** 남은(선택): #7 비수직 step-bore subsumption seed-only(니치 edge).

**2차 적대적 리뷰 — 세션 전체 지오메트리/인식기 (2026-06-18):** grammar 리뷰가 6버그를 잡았으니 나머지 세션 코드(grounding 3·ParamClamp·폰빌더·cap/lift)도 12-에이전트 리뷰. **또 실버그 색출**: 🔴 **liftRecoveredStep가 측정 챔퍼>2.0mm를 0.5로 무음 손상**(3mm→0.5, recognizer 밴드 [0.05,50]) → 상한 클램프 제거(측정값 보존, sub-0.1 floor 만 유지), recognizer_test 의 3.0→0.5 단언을 보존 검증으로 교체. 🟠 **ParamClamp 0.4mm DFM floor 가 유효 0.35mm 벽 over-clamp**(crash 아님, validate 는 warning) → floor 제거(crash 가드 wMax 만). 🟠 **DFM-018 이 tear land 아닌 groove 폭 측정** → groove 폭 + **lens surround(inner−camera_dia)/2** 둘 다 검사(ring↔camera offset 매칭). 🟠 **auto_rib slab 게이트가 얇은 본체의 진짜 rib false-reject** → 비율+절대(>3mm) 결합(thin_plate 4mm 여전히 kill, fpscan 불변). 회귀 테스트: PreservesMeasuredChamfer/PreservesValidThinWall. **반증**: tube_swage U-span sliver(다운스트림 필터가 거름), bore_cylindrical cyl_wrap_rad(결함 없음 확인). auto_rib 3D-dist/replay-shadow(#4b/#5)는 capped 라 production 영향 미미 → 보류. **2회 적대적 리뷰로 총 10개 무음 결함 색출·수정 — RE 복원 경로 신뢰성 확보.**
