...
**grammar 컴파운드 actionable 화 — 인식→편집→재생성 루프 완성 (2026-06-17, 미션 페이오프):** 추천에 따라 grammar 작업의 빠진 절반 보완. grammar 4패턴은 **인식만** 됐음(apply 부재 → inferProcessPlan Unknown 버킷, replay 시 filterToDispatched 가 드롭). 플래그십 **`bolt_circle_pattern` 을 생성형 스킬로 구현**(`src/skills/bolt_circle_pattern.{hpp,cpp}`): apply 가 drill_hole::apply 를 hole_count 회 합성(entry/through 해석·체인계약 상속), validate(≥3홀·dia·DFM-003 피치갭), recognize 는 {}(인식은 koo_re 그래머). Executor dispatch 배선(parseBoltCirclePattern: axis_dir 로 entry 면 유도) + skills CMake + check_feature_chain 위임 화이트리스트. **recovered_params 키가 그래머 출력과 일치 → 인식된 볼트서클이 replayable·editable 스텝이 됨.** 검증 `tests/skills/bolt_circle_pattern_test`(5, 신규 ctest): 생성(유효 ring·재료제거) / **편집(hole_count 4→6 재생성, 더 많은 재료 제거)** / validate 거부 / **STEP 왕복 재인식(hole_count=4)** / **Executor dispatch 성공**. **의의: 인식된 컴파운드를 파라메트릭하게 편집·재생성 — 일반화된 설계 자동화의 실증.** 남은: grid/linear/step-bore 도 생성형으로(같은 패턴) + inferProcessPlan 에서 컴파운드가 구성 drill 스텝 subsume(중복 cut 방지, 단 동일 geometry 재cut 은 무해).
